...
void draw_pie_slice(float angle, float origx, float origy)
{
  PVector island = new PVector(origx, origy);

  /* define lines */
  float minus_angle = angle - 5.5;
  float plus_angle = angle + 5.5;
  if (angle == 180.0)
    plus_angle = -175.0;
...
{ 
  ...
  */
  if (selectedMunicipality != "")
  {
    ...
    float weight = baseWeight + 0.1 * municipalities.size();
    strokeWeight(weight);
    stroke(0, 20, 0);
...
void draw() {
  ...
  color lowestPopulationColor = color(194, 240, 207);
  color highestPopulationColor = color(2, 48, 15);
  ...
  strokeWeight(1);
  rectMode(CORNERS);
  mapX1 = panZoomMap.longitudeToScreenX(138.0);
  mapY1 = panZoomMap.latitudeToScreenY(5.2);
  mapX2 = panZoomMap.longitudeToScreenX(163.1);
  mapY2 = panZoomMap.latitudeToScreenY(10.0);
  ...
  for (int i = 1; i < 11; i++)
  {
    strokeWeight(baseWeight + 0.1*i*step);
